  ...
  exports.Identifier = (function(_super) {
    ...
    Identifier.prototype.compile = function(program) {
      ...
      return {
        execute: function() {
          var name;
          name = _this.children[0];
          if (program.state.variables[name]) {
            return program.state.variables[name].value;
          } else {
            program.state.variables[name] = program.state.scope.lookup(name).as_options();
            if (program.state.variables[name].value === void 0) {
              program.state.variables[name].value = Number.NaN;
            }
            return program.state.variables[name].value;
          }
        },
